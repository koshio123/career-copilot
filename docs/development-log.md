...
Signed-in browsing with a type-safe API client; login → protected page → API
call → logout works end to end.

### What was built

**Stack (ADR-0011)** — Tailwind v4 (`@tailwindcss/vite`), hand-rolled components
in `src/components/ui.tsx`, React Router v7 (library mode), TanStack Query v5,
React Hook Form + Zod, `openapi-fetch`, `@sentry/react` (no-op without a DSN).

**Typed API contract** — `backend/scripts/dump_openapi.py` + `make openapi` write
the committed `backend/openapi.json`; `pnpm run gen:api` regenerates
`frontend/src/api/schema.ts` (`openapi-typescript`). CI fails if either drifts
(`git diff --exit-code`).

**API client** — `src/api/client.ts`: `createClient<paths>({ credentials:
'include' })` + a middleware that echoes the `cc_csrf` cookie into `x-csrf-token`
on unsafe methods; `unwrap()` turns an openapi-fetch result into data or throws a
normalised `ApiError` (from the RFC 9457 body).

**Auth** — `src/auth/hooks.ts`: `useMe` (query, 401 → `null`), `useRequestOtp` /
`useVerifyOtp` (mutations; verify seeds the `me` cache), `useLogout`.

**Routing** — `src/App.tsx`: `/login` (public), everything else behind
`ProtectedRoute` (redirects to `/login` with a `from`) inside `AppLayout`
(header with email + sign-out). `LoginPage` is a two-step form (email → 6-digit
code) with per-step Zod validation and a shared error banner. Each step's
`<form>` carries a distinct `key` so React unmounts one and mounts the other —
without it the two forms reconcile to the same DOM nodes and the uncontrolled RHF
`<input>` keeps whatever the user typed in the previous step (the 6-digit field
would show the email address).

**Tests** — Vitest + Testing Library (`LoginPage.test.tsx`, incl. a regression
test for the step-swap input bug); Playwright (`e2e/login.spec.ts`, API mocked
with `page.route`) drives the full sign-in / sign-out flow. New `e2e` CI job
installs Chromium and runs it. `setupTests.ts` makes the API client usable under
jsdom: it stubs `Request` to resolve the app's same-origin relative paths
(`/api/v1/...`, no `baseUrl`) against `http://localhost:3000`, and installs a
single `fetch` mock up front — openapi-fetch binds `globalThis.fetch` at import,
so a per-test `stubGlobal` would never be seen — defaulting to 401 and refined
per test with `vi.mocked(fetch).mockImplementation(...)`.

**Manual-test doc** — `docs/manual-testing.md` gains a "SPA を起動" step
(`make web`, Vite `/api` proxy) and ch. 6: a browser walkthrough of login →
protected page → reload → sign-out plus failure/edge cases (Zod client
validation, wrong-code banner, `/login` while signed in, unknown path, API down,
cookie flags) and an optional `pnpm build && pnpm preview` check.

### Verification

| Gate | Result |
|---|---|
| `make lint` | ruff + mypy ✓ ; eslint + tsc (project refs) ✓ |
| `make test` | pytest 19 ✓ ; vitest 3 ✓ |
| `pnpm run build` | 390 kB / 122 kB gzip ✓ |
| `pnpm run e2e` | Playwright login flow ✓ |
| real integration | backend :8000 + Vite :3000; `/api/v1/*` proxies, SPA loads, OTP request → 202 |

### Deviations from `development-plan.md`

- "Sign up" is not a separate flow — first OTP verify creates the account
  (ADR-0010), so login and sign-up are the same screen.
- "Session refresh" is server-side (sliding expiry on the backend) + `useMe`
  staleTime; nothing explicit on the client.

### Follow-ups

- Playwright currently mocks the API; a full-stack E2E (real backend + LocalStack)
  can be a separate, slower CI job later.
- `openapi.json` is committed and regenerated by `make openapi` — remember to run
